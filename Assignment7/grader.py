...
def test_1a():
    cp = submission.ConditionalProb()
    kargs = dict(delta=0.5, epsilon=0.8, eta=0.2)
    cp.setBinaryEnv(**kargs)    
    cp.observeSeq([0])
    pred = cp.getCondProb()
    grader.requireIsEqual({0: 0.8, 1: 0.2}, pred, 0.00001)

grader.addBasicPart('1a-1-basic', test_1a, 0, maxSeconds=1, description="Basic test for problem-1a")

def test_1a_2():
    cp = submission.ConditionalProb()
    kargs = dict(delta=0.8, epsilon=0.8, eta=0.8)
    cp.setBinaryEnv(**kargs)    
    cp.observeSeq([1,1])
    pred = cp.getCondProb()
    grader.requireIsEqual({0: 0.5517241379310344, 1: 0.4482758620689656}, pred, 0.00001)

grader.addBasicPart('1a-2-basic', test_1a_2, 1, maxSeconds=1, description="Basic test for problem-1a")
...
def test_1a_3():
    ...
    def get_cmp_gen():
        for _ in range(20):
            ...
            cp.setBinaryEnv(**kargs)
            observations = [random.randint(0, 1) for _ in range(random.randint(5,10))]
            cp.observeSeq(observations)
            pred = cp.getCondProb()
            if solution_exist:
                sol = solution.ConditionalProb()
                sol.setBinaryEnv(**kargs)
                sol.observeSeq(observations)
                answer = sol.getCondProb()
                yield grader.requireIsEqual(answer, pred, 0.00001)
    all(get_cmp_gen())

grader.addHiddenPart('1a-3-hidden', test_1a_3, 3, maxSeconds=1, description="Hidden test for problem-1a")
...
grader.addBasicPart('2a-0-basic', test_2a, 1, description="2a basic test for emission probabilities")
...
grader.addHiddenPart('2a-2-hidden', test_2a_2, 2, description="2a advanced test for emission probabilities")
...
grader.addBasicPart('3a-0-basic', test_3a, 1, description="test correctness of elapseTime()")
...
grader.addHiddenPart('3a-2-hidden', test_3a_2, 2, description="advanced test for emission AND transition probabilities, strict time limit", maxSeconds=5)
